...
public class Problem107 {
    ...
    static class MinimumSpanningTree {
        ...
        public MinimumSpanningTree(int[][] mst, int weight) {
            this.mst = mst;
            this.weight = weight;
        }
    }
    ...
    public static void main(String[] args) {
        String[] lines = network_text.split("\n");
        int[][] edges = new int[lines.length][lines.length];
        int totalWeight = 0;
        ...
        MinimumSpanningTree tree = minimumSpanningTree(edges);
        int difference = totalWeight - tree.weight;
        System.out.println("Difference: " + difference);

        System.out.println("Solution:");
        for (int i = 0; i < tree.mst.length; i++) {
            System.out.println(Arrays.toString(tree.mst[i]));
        }
    }

    public static boolean isInvalidWeight(int weight) {
        return weight < 0;
    }

    public static MinimumSpanningTree minimumSpanningTree(int[][] graph) {
        int[][] minimumTree = new int[graph.length][graph.length];

        boolean[] joinedVertex = new boolean[graph.length];
        int totalWeight = 0;

        joinedVertex[0] = true; // start at first vertex

        // We need to add graph size - 1 connections
        for (int iteration = 0; iteration < graph.length - 1; iteration++) {
            
            int weight = Integer.MAX_VALUE;
            int beginningVertex = -1;
            int endingVertex = -1;
            for (int startingVertex = 0; startingVertex < graph.length; startingVertex ++) {
                if (!joinedVertex[startingVertex]) {
                    continue;
                }

                for (int newVertex = 0; newVertex < graph.length; newVertex ++) {
                    if (joinedVertex[newVertex]) {
                        continue;
                    }

                    int candidate = graph[startingVertex][newVertex];

                    if (isInvalidWeight(candidate)) {
                        continue;
                    }

                    if (candidate < weight) {
                        weight = candidate;
                        beginningVertex = startingVertex;
                        endingVertex = newVertex;
                    }
                }
            }

            joinedVertex[endingVertex] = true;
            totalWeight += weight;

            minimumTree[beginningVertex][endingVertex] = weight;
            // minimumTree[endingVertex][beginningVertex] = weight;
        }
        return new MinimumSpanningTree(minimumTree, totalWeight);
    }

    private static final String mini = "-,16,12,21,-,-,-\n" +
    "16,-,-,17,20,-,-\n" +
    "12,-,-,28,-,31,-\n" +
    "21,17,28,-,18,19,23\n" +
    "-,20,-,18,-,-,11\n" +
    ...
    private static final String network_text = "-,-,-,427,668,495,377,678,-,177,-,-,870,-,869,624,300,609,131,-,251,-,-,-,856,221,514,-,591,762,182,56,-,884,412,273,636,-,-,774\n" +
